...
file_dir = "data/processed/quality/"
...
for filename in files:
    ...
    sample_id = int(barcode.split("_")[1])
    alignments = list(AlignmentFile(os.path.join(filename, "output", barcode + ".trimmed.rg.sorted.bam")).fetch())
    consensus = list(skbio.read(os.path.join(filename,"output", barcode + ".consensus.fasta"), format="fasta"))[0]
    coverage.append({
        "sample_id": sample_id,
...
coverage.to_csv("analysis/mean_coverage_9.csv", index=False)
...
root = root_scalar(f, bracket=[100,400]).root
with Plotter(filename="analysis/mean_aligned_coverage_vs_completeness_9.svg", figsize=(12,12)) as ax:
...
with Plotter(filename="analysis/mean_unaligned_coverage_vs_completeness_9.svg", figsize=(12,12)) as ax:
...
with Plotter(filename="analysis/aligned_vs_unaligned_reads.svg_9", figsize=(9,9)) as ax:
    coverage.plot.scatter("mean_coverage","mean_aligned_coverage", c="completeness", ax=ax, colormap='bwr')
